...
adv_train_df = pd.read_csv(r'/home/linuxu/Desktop/final_project/Adversarial-Data-Encryption/data/reconstruction/fgsm/df_results.csv')
image_root = r'/home/linuxu/Desktop/final_project/Adversarial-Data-Encryption/data'   
adv_train_df['adv_path'] = 'reconstruction/fgsm/train/' + adv_train_df['image'].str.lstrip("./")
adv_train_df['clean_path'] = 'nih/' + adv_train_df['image'].str.lstrip("./") 

adv_test_df = pd.read_csv(r'/home/linuxu/Desktop/final_project/Adversarial-Data-Encryption/data/reconstruction/fgsm/df_test_results.csv')
image_root = r'/home/linuxu/Desktop/final_project/Adversarial-Data-Encryption/data'   
adv_test_df['adv_path'] = 'reconstruction/fgsm/train/' + adv_test_df['image'].str.lstrip("./")
...
def kl_divergence(mu, logvar, reduction="mean"):
    ...
    kl = kl.sum(dim=1)  # per-sample
    return kl.mean() if reduction == "mean" else kl.sum()

# def build_optimizer(model):
#     # smaller lr for pretrained encoder, larger for new heads
#     enc_params = list(model.feature_extractor.parameters())
#     new_params = list(model.fc_mu.parameters()) + list(model.fc_logvar.parameters()) + \
#                  list(model.decoder_fc.parameters()) + list(model.decoder.parameters()) + \
...
def model_train(model, epoch, train_loader, optimizer, lam, beta, ckpt_dir):
    ...
    for x_adv, x_clean, y in train_loader:
        ...
        totals["kl"]+=kl.item(); totals["ce"]+=ce.item(); totals["acc"]+=acc
    ...
    {"epoch": int(epoch), "split": "train",
     ...
     "kl": float(avg["kl"]), "ce": float(avg["ce"]), "acc": float(avg["acc"])},
...
ckpt_dir = r"/home/linuxu/Desktop/final_project/Adversarial-Data-Encryption/data/reconstruction/fgsm/models/ver3_recon_5/"
...
def load_metrics(path):
    ...
    return float(obj["loss"]), float(obj["recon"]), float(obj["kl"]), float(obj["ce"])
...
train_losses, train_recon, train_kl, train_ce= zip(*train_metrics)
val_losses, val_recon, val_kl, val_ce = zip(*val_metrics)
...
# ------- plot -------
plt.figure(figsize=(8,4))
plt.plot(range(1, len(train_ce)+1), train_kl, label="Training", marker="o")
plt.plot(range(1, len(val_ce)+1),   val_kl,   label="Validation", marker="o")
plt.xlabel("Epoch")
plt.ylabel("Loss")
...
selected_ckpt_dir = r"/home/linuxu/Desktop/final_project/Adversarial-Data-Encryption/data/reconstruction/fgsm/models/ver2/train_epoch16.pt"
